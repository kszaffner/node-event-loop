...
PATTERNS = [
    ...
    ("Hardcoded API key",           r'(?i)(?:api_key|apikey|api_secret)\s*[:=]\s*["\']?[^\s"\']{8,}["\']?'),
    ("Hardcoded secret",            r'(?i)(?:secret_key|client_secret)\s*[:=]\s*["\']?[^\s"\']{8,}["\']?'),
]

SAFE_SUFFIXES = (".example", ".sample", ".template")
SAFE_NAMES = ("secret-guard.py",)
...
def main():
    ...
    if hits:
        ...
        lines += [
            "",
            "Fix: store secrets in environment variables instead of source files.",
